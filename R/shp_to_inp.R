...
#' 
#'  @param path_options Name (incl. path) to a .txt file with SWMM sections. Write section name lower case and in []. The following sections are allowed: options, report, raingages, evaporation, pollutant, landuse, buildup, washoff, coverages. 
...
#'  @param path_pump Name (incl. path) to a .shp file with line features. All parameters must be given: Name, FromNode, ToNode, Pcurve, status, Startup, Shutoff.
...
#'  @rdname shp_to_inp
#'  @export
shp_to_inp <- function(path_options = NULL, path_polygon = NULL, subcatchment_typologies = NULL, path_point = NULL, junction_parameters = NULL, path_outfall = NULL, path_line = NULL, conduit_material = NULL, path_timeseries = NULL, infiltration = NULL, path_pumps = NULL, path_pump_curve = NULL, path_weirs = NULL, path_storages = NULL, path_storage_curve = NULL){
  ...
  if(missing(path_options)){
    ...
    for(i in 1:length(section_lines)){
      ...
      list_of_sections[[i]] <- tidyr::gather(list_of_sections[[i]],Variable, Value) %>%
        tidyr::spread_(names(.)[1], "Value") %>% .[,cols]
  ...
  res <- list_of_sections %>% 
    # define classes
    purrr::imap(function(.x, .y) {class(.x) <- c(class(.x), .y);return(.x)}) %>%
