...
#' 
#' 
#' @title Read time series data from SWMM out file
#' @param file The file to be read.
#' @param iType Sets the result type: 0 for Subcatchments,
#' 1 for nodes, 2 for links, 3 for system variables. Leave empty for retrieving elements.
#' @param object_name Sets the objects of which time series data is returned. Leave empty for retrieving elements..
#' @param vIndex Sets the variables to be read (s. Details). Leave empty for retrieving elements.
...
#' vIndex choices:
#' 
#' Code number of each \strong{subcatchment} variable:
#' \itemize{
...
#' }
#' 
#' Code number of each \strong{node} variable:
#' \itemize{
...
#' }
#' 
#' Code number of each \strong{link} variable:
#' \itemize{
...
#' }
#' 
#' Code number of each \strong{system-wide} variable:
#' \itemize{
...
read_out <- function(file="",
  ...
  # for each iIndex, i.e. each subcatchment or nodes or links ...
  list_of_xts <- lapply(seq_len(length(iIndex$iIndex)), FUN = function(x) {

    # get vIndex, i.e. the type of variables 
    vIndex <- .get_vIndex(iType = iType, vIndex = vIndex)
